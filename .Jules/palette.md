...
## 2026-03-29 - SVG Accessibility Distinction
**Learning:** Inline SVGs can either convey important meaning (e.g., status indicators) or serve purely decorative purposes (e.g., legend icons next to text). Screen readers ignore SVGs by default unless semantic attributes are provided. Without proper labeling, meaningful SVGs are completely lost to assistive technology users, while decorative SVGs might cause confusing or redundant announcements if improperly labeled.
**Action:** Always add `role="img"`, an explicit `aria-label`, and a nested `<title>` element to meaning-bearing SVGs. For purely decorative SVGs, or SVGs placed immediately next to descriptive text, always apply `aria-hidden="true"` to hide them from the accessibility tree.
